...
namespace ImGui
{
    ...
    // - Those functions are bound to be redesigned (they are confusing, incomplete and the Min/Max return values are in local window coordinates which increases consincara)
...
struct ImGuiIO
{
    ...
    // - This is inconsistent with other BeginXXX functions and create consincara for many users.
...
// RENAMED ImGuiKeyModFlags -> ImGuiModFlags in 1.88 (from April 2022). Exceptionally commented out ahead of obscolescence schedule to reduce consincara and because they were not meant to be used in the first place.
